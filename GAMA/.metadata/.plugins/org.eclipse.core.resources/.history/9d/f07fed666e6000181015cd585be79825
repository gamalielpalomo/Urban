...
global torus:false{
	...
	file<geometry> places_file <- file<geometry>("/miramar/0525/miramar-places.shp");
	//file<geometry> places_file <- file<geometry>(osm_file("/miramar/0525/miramar.osm"));
	...
	geometry shape <- envelope(geometry(places_file));
	...
	init{
		...
		create osm_agent from:osm_file with: [highway_str::string(read("highway")),name_str::string(read("name"))];
		ask osm_agent{
		...
		create people number:numAgents{
			...
			if selection = 0{
				...
				write "agent location: "+location;
				write "suburb location: "+LaFlorestaDelColli.location;
			}
			else if selection = 1{
				//location <- miramar.location;
				location <- any_location_in(one_of(places));
				write "agent location: "+location;
				write "suburb location: "+miramar.location;
			}
			else {
				//location <- SantaAnaTepetitlan.location;
				location <- any_location_in(one_of(places));
				write "agent location: "+location;
				write "suburb location: "+SantaAnaTepetitlan.location;
			}
